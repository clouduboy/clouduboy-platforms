...
int rocket_y;
int turret_x;
byte invaders[INVADER_ALIENS * INVADER_WAVES];
int total_invaders;
int game_over;
int defeat_after = 0;
int victory_after = 0;
...
// set fill style to game.fillStyle = "white";
{
arduboy.setCursor( WIDTH / 2 -(strlen(" OH NO!")*6/2-1), HEIGHT / 2 );
arduboy.print( " OH NO!" );
};
...
if (defeat_after > 0 && (_microcanvas_frame_counter - defeat_after) > 3 * 60) {
  // set fill style to game.fillStyle = "white";
  {
arduboy.setCursor( WIDTH / 2-(strlen("press [A] to retry")*6/2-1), HEIGHT / 4 * 3 );
...
// set fill style to game.fillStyle = "white";
{
arduboy.setCursor( WIDTH / 2 -(strlen(" HUMANITY PREVAILS!")*6/2-1), HEIGHT / 2 );
...
if (fall < HEIGHT) {
  ...
  // set fill style to game.fillStyle = "white";
  {
  arduboy.setCursor( WIDTH / 2-(strlen("[A] to play again")*6/2-1), HEIGHT / 4 * 3 );
...
while (y < INVADER_WAVES) {
  ...
  int d_y = (invasion + y * (GFX_INVADER_HEIGHT + 3));
...
arduboy.fillRect( (WIDTH / 2 - 6), 0, 12, 8, BLACK );
// set fill style to game.fillStyle = "white";
{
sprintf(_microcanvas_textbuffer, "%d", game_timer_animation_remaining() / 60);
arduboy.setCursor( (WIDTH / 2 - strlen(_microcanvas_textbuffer)*6/2+1), 0 );
...
////// FUNCTION BODY //////
return max(0, (60 - (_microcanvas_frame_counter - defender_win_animation_start)) );
...
void setup() {
  if (!_microcanvas_frame_counter) arduboy.begin();
  _microcanvas_frame_counter = 0;

////// CUSTOM SETUP //////
if (!arduboy.cpuLoad()) { // cpuLoad will only be 0 on boot, beforefirst call to arduboy.nextFrame
  arduboy.clear();
  arduboy.drawBitmap( (WIDTH/2-64/2), (HEIGHT/2-64/2), sjs_intro, 64,64, WHITE );
  arduboy.display();
  delay(2000);
}
turret_x = WIDTH / 2;
